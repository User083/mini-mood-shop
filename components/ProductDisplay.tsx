...
import { filters, sortOptions } from "@/utils/constants";
import { Loader } from ".";

function SortByPrice(array: SetStateAction<any[]>, direction: string) {
  let newArray: any[] = [];
  array.forEach((item) => {
    newArray.push(item);
  });
...
const ProductCard = ({ id, image, title, category, price }: IProps) => {
  function UpdateCart(array: any[], item: any) {
    let newCart = array;
  ...
  return (
    <article id={id} className="bg-secondary rounded my-5 md:max-w-[300px]">
...
const ProductDisplay = (props) => {
  ...
  const getProducts = async (params: type) => {
